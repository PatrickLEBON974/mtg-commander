  ...
  <Transition name="result-backdrop">
    ...
    >
      ...
      <Transition :name="slideFromRight ? 'result-slide-right' : 'result-slide-left'">
        <div
          v-if="isOpen"
          class="result-panel relative z-10 flex w-[75%] flex-col gap-2 rounded-2xl p-3"
          :class="slideFromRight ? 'mr-2' : 'ml-2'"
        >
          <!-- Winner -->
          <button class="result-option result-winner" @click="handleResult('winner')">
            <IconCrown :size="22" color="#f0d078" />
          ...
          <button class="result-option result-eliminated" @click="handleResult('eliminated')">
            <IconSkull :size="22" class="text-life-negative" />
          ...
          <button class="result-option result-surrender" @click="handleResult('surrender')">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" class="text-white/60">
          ...
          <button class="result-option result-draw" @click="handleResult('draw')">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" class="text-white/50">
          ...
          <!-- Cancel -->
          <button
            class="mt-1 rounded-lg bg-white/5 py-2 text-center text-xs font-medium text-white/40 active:bg-white/10"
            @click="$emit('close')"
          >
            {{ t('common.cancel') }}
          </button>
